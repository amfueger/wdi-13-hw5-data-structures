...
//Create a new array called bondTitles with only the titles of the Bond films, and console.log the new array.  Commit: "bondTitles array"
const bondTitles = [];
for (let i = 0; i < bondFilms.length; i++) {
	//Watch your god damn operaters. Stop making things equal to, when you're not looking to go past the exclusivity. 
...
for (let i = 0; i < bondFilms.length; i++) {
	...
	//i is the counter through the bond films, and there's some that don't get included. So I can't use i on oddBonds. 
	}
...
let sum = 0;
for (let i = 0; i < bondFilms.length; i++) {
	cumulativeGross.push(bondFilms[i].gross.replace('$', '').replace(',', '').replace(',', '').replace(',', ''));
//could have used ([^0-9.-]+/g,"") <-- instead of replacing things individually.
} 
for (let j = 0; j < cumulativeGross.length; j++) {
	cumulativeGross[j] = parseInt(cumulativeGross[j]);
	sum = cumulativeGross.reduce((total, amount) => total + amount);
} //the above was a waste of time, because apparently other people did it more simply, but whatever
console.log(sum);
/*
